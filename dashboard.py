...
<style>
    ...
    .badge-WARNING  { background:#2d2a1f; color:#d29922; }
    .badge-ERROR    { background:#2d1f1f; color:#f85149; }
    .badge-SUCCESS  { background:#1f3d2e; color:#3fb950; }
    .badge-DEBUG    { background:#1a1f2e; color:#8b949e; }
...
class LogManager:
    """Manages logs for display in dashboard"""
    _instance = None
    _logs = deque(maxlen=1000)  # Keep last 1000 logs
            ...
            for path in common_paths:
                if os.path.exists(path):
                    if mt5.initialize(path):
                        log_manager.add_log("SUCCESS", f"MT5 initialized with: {path}")
                        break
            
            if not mt5.initialize():
                log_manager.add_log("ERROR", "MT5 initialization failed")
                return False, "MT5 initialization failed. Make sure MT5 is running."
        
        account_info = mt5.account_info()
        if account_info is None:
            mt5.shutdown()
            log_manager.add_log("ERROR", "MT5 not logged in")
            return False, "MT5 not logged in. Please login to MT5 first."
        
        log_manager.add_log("SUCCESS", f"MT5 connected: Account {account_info.login}")
        return True, "Connected"
        
    except Exception as e:
        log_manager.add_log("ERROR", f"MT5 error: {str(e)}")
...
def get_mt5_trades(days_back: int = 7):
    ...
    try:
        ...
        account_info = mt5.account_info()
        if account_info is None:
            log_manager.add_log("WARNING", "MT5 account not logged in")
            mt5.shutdown()
            return []
        ...
        if positions:
            log_manager.add_log("INFO", f"Found {len(positions)} open positions")
            ...
            log_manager.add_log("INFO", "No open positions found")
        ...
        if deals:
            log_manager.add_log("INFO", f"Found {len(deals)} deals in history")
            position_deals = {}
            for deal in deals:
        ...
        log_manager.add_log("ERROR", f"Error reading from MT5: {str(e)}")
        return []
        ...
        if mt5_initialized:
            try:
                mt5.shutdown()
                log_manager.add_log("DEBUG", "MT5 shutdown")
            except:
                pass
...
with st.sidebar:
    ...
    if st.button("🔄 Refresh now"):
        st.cache_data.clear()
        log_manager.add_log("INFO", "Manual refresh triggered")
        st.rerun()
...
# Log controls
col_log1, col_log2, col_log3, col_log4 = st.columns([2, 1, 1, 1])
...
# Display logs
logs = log_manager.get_logs(limit=log_limit)
...
if logs:
    ...
    st.caption(f"Showing {len(logs)} logs (filtered from {len(log_manager._logs)} total)")
else:
